...
import { academicYears, periods, levels, subjectParents } from "@/polymet/data/school-data"
...
export function AcademicYearsPage() {
  ...
  const handleConfigure = (yearId: string) => {
    navigate(`/admin/academic-year/${yearId}/config`)
  }
  ...
  const handleCreateYear = (data: {
    ...
    // TODO: Implémenter la création d'année
  }
  ...
  const yearCards = academicYears.map(year => {
    ...
    return {
      ...
      stats: year.status !== 'archived' ? {
        periods: { 
          current: yearPeriods.length, 
          total: 4, 
          complete: yearPeriods.length >= 4 
        },
        classes: { 
          current: yearLevels.length, 
          complete: yearLevels.length > 0 
        },
        subjects: { 
          current: yearSubjectParents.length, 
          complete: yearSubjectParents.length > 0 
        }
      } : undefined
    }
  })
